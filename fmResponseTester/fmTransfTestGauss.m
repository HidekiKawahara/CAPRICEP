function output = fmTransfTestGauss(fo, fMcent, outDirName, foWrapper)
%   Response measurement of pitch extractors to FM test signal
%      output = fmTransfTestGauss(fo, fMcent, outDirName, foWrapper)
%
%  Argument
...
tmp = sum(orthSeq, 2);
ttw = ((1:fftlSeg)' - fftlSeg/2)/(fftlSeg/2);
...
title(titleStr + " fo:" + num2str(fo, '%7.1f') + " Hz, FMdpth:" ...
     + num2str(fMcent, '%7.1f') + " cent, SIG:" + carrierType);
xlabel('frequency (Hz)')
ylabel('gain (dB)')
legend('LTI', 'TV-rand', 'non-LTI', 'location', 'southeast', 'fontsize', 15);
fNameOut = filePrefix + datestr(now,30) + "d";
drawnow;
