function output = pitchOpenSmileS(xa, fs)
...
output = struct;
%[f0, loc] = pitch(xa, fs);
audiowrite("testSignal.wav", xa/max(abs(xa))*0.8, fs,"BitsPerSample",24);
%!PATH=$PATH:~/Downloads/opensmile-3.0.1-macos-x64/bin/
%!SMILExtract -C prosodyShs.conf -I testSignal.wav -O testSignal.htk
!~/Downloads/opensmile-3.0.1-macos-x64/bin/SMILExtract -C ~/Downloads/opensmile-3.0.1-macos-x64/config/prosody/prosodyShs.conf -I testSignal.wav -O testSignal.htk
htkData = v_readhtk('testSignal.htk');
output.fo = htkData(:,1);
output.tt = (1:length(output.fo))'/100;
output.titleStr = "openSMILEs ";
output.filePrefix = "pOSMILEs";
end